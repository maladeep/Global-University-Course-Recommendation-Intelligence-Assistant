...
if page == "Home":
    ...
    **ℹ️ Usage:** Use the dropdown menu on the left to navigate between pages.
    ...
    st.warning(
        "The predictions generated by this application are based on a dataset of limited size. As such, the results should be interpreted with caution. The findings are preliminary and may not fully capture the complexity of all factors influencing students' decisions to study abroad. We recommend using the insights as a guideline rather than a definitive conclusion. For more comprehensive results, consider consulting additional data sources or conducting further research."
    )
